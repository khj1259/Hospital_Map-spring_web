...
function showMarker(data) {
         ...
         if(result.response.body.totalCount!=1){//item이 배열
         ...
         for (var i = 0; i < MAX; i++) {
            positions.push({
               title : item[i].yadmNm,
               latlng : new daum.maps.LatLng(item[i].YPos,
                     item[i].XPos)
            })
         ...
         for (var i = 0; i < positions.length; i++) {
           ...
           bounds.extend(positions[i].latlng);//화면조정을 위해서 해당 객체에 좌표들을 저장한다.
            ...
            var content = '<div class="wrap">' + '    <div class="info">'
                  ...
                  + '(m)</div></div><div class="search00">'
                  + '               <div class="search0"><div style="width:100%;" class="img0"><img style="align:center;" src="./js/road.png"/></div>'
                  + '<a href="http://map.daum.net/link/to/'+positions[i].title+','+positions[i].latlng.jb+','+positions[i].latlng.ib+'">길찾기</a></div>'
                  + '               <div class="search1"><div style="width:100%;" class="img0"><img style="align:center;" src="./js/view.png"/></div>'
                  + '<a href="http://map.daum.net/link/to/'+positions[i].title+','+positions[i].latlng.jb+','+positions[i].latlng.ib+'">로드뷰</a></div>'
                  + '               <div class="search2"><div style="width:100%;" class="img0"><img style="align:center;" src="./js/share.png"/></div>'
                  + '<a href="http://map.daum.net/link/to/'+positions[i].title+','+positions[i].latlng.jb+','+positions[i].latlng.ib+'">공유</a></div>'
                  + '            </div></div>'
                  + '    </div>' + '</div>';
            ...
            //마커에 클릭이벤트 달기
            daum.maps.event.addListener(marker, 'click', makeClickListener(overlay, i, marker));
